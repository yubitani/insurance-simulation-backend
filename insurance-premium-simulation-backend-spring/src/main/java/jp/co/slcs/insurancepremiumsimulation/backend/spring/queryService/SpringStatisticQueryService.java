...
public class SpringStatisticQueryService implements StatisticQueryService {
	 ...
	 * @return 商品名
	 * 
	 * TODO 商品コードと商品名をベタ書きしているので修正要。
	 * TODO MapからListへの変換や商品コードから商品名への変換はusecaseで実施すべき？
	 */
	private static String convertToProductName(String productCode){	
    	...
    	String result = productNameMap.entrySet().stream()
	...
	public List<TotalDataRow> fetchTotalData(){
		...
		List<String> productCodeList = Arrays.asList("DeathInsurance-001","MedicalInsurance-001","CancerInsurance-001");
		productCodeList.stream().forEach(
				pc -> result.add(TotalDataRow.builder()
									.productName(convertToProductName(pc))
									.number(simulationDao.countByProductCode(pc))
									.build()));
		return result;
	}
	 ...
	 * TODO Map<String, Long> -> List<TransitionDataRow>はstaticメソッド化する。
	 */
	@Override
	public List<TransitionDataRow> fetchTransitionDataPerDay(){
		...
		if(list != null) {
			...
			List<TransitionDataRow> result = rawData.entrySet().stream().map(
					e -> TransitionDataRow.builder()
							.dateTime(e.getKey())
							.number(e.getValue())
							.build())
				.collect(toList());
			...
			result = result.stream()
					.sorted(Comparator.comparing(TransitionDataRow::getDateTime))
					.collect(toList());
