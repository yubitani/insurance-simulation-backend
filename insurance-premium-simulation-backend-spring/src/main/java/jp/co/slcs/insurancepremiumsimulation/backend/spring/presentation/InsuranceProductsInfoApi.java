...
import jp.co.slcs.insurancepremiumsimulation.backend.spring.presentation.model.InsuranceProductInfoResDto;
import jp.co.slcs.insurancepremiumsimulation.backend.spring.presentation.model.InsuranceProductInfoResDto.Benefit;
import jp.co.slcs.insurancepremiumsimulation.backend.spring.presentation.model.InsuranceProductInfoResDto.BenefitChoices;
import jp.co.slcs.insurancepremiumsimulation.backend.spring.presentation.model.InsuranceProductInfoResDto.Option;
import jp.co.slcs.insurancepremiumsimulation.backend.spring.presentation.model.InsuranceProductInfoResDto.PeriodOfInsurance;
...
public class InsuranceProductsInfoApi {
    ...
    public List<InsuranceProductInfoResDto> getInsuranceProductInfo(){
        return usecase.invoke().stream().map((insuranceInfo -> {
            return InsuranceProductInfoResDto.builder()
                                             .productCode(insuranceInfo.getProductCode())
                                             .name(insuranceInfo.getProductName())
                                             .benefit(Benefit.builder()
                                                             .benefitName(insuranceInfo.getBenefit().getBenefitName())
                                                             .benefitConditionList(insuranceInfo.getBenefit().getBenefitConditionList().stream().map(benefitChoices -> {
                                                                 return BenefitChoices.builder()
                                                                                      .benefitCode(benefitChoices.getBenefitCode())
                                                                                      .benefitChoicesName(benefitChoices.getBenefitChoicesName())
                                                                                      .build();
                                                             }).collect(Collectors.toList()))
                                                             .build()
                                             )
                                             .periodOfInsuranceList(insuranceInfo.getPeriodOfInsuranceList().stream().map(periodOfInsurance -> {
