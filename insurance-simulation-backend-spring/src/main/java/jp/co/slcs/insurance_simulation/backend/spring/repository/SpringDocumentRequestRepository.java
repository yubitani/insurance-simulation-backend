...
public class SpringDocumentRequestRepository implements DocumentRequestRepository {
  ...
  private CustomerDaoEntity buildCustomerDaoEntity(@NonNull DocumentRequest documentRequest,
      boolean isNew) {
    return CustomerDaoEntity.builder().name(documentRequest.getName())
        .kana(documentRequest.getNameKana()).birthday(documentRequest.getBirthday().getBirthday())
        .sex(documentRequest.getSex().toString()).zipcode(documentRequest.getZipCode())
        .address(documentRequest.getAddress()).telNo(documentRequest.getTellNo())
        .mail(documentRequest.getMailAddress()).isNew(isNew).build();
  }

  private DocumentRequestDaoEntity buildDocumentRequestDaoEntity(
      @NonNull DocumentRequest documentRequest, Integer customerId, boolean isNew) {
  ...
  private SimulationDaoEntity buildSimulationDaoEntity(@NonNull DocumentRequestProduct product,
      String receiptNo, boolean isNew) {
    return SimulationDaoEntity.builder().productCode(product.getProductCode())
        .benefitCode(product.getBenefitCode())
  ...
  private SimulationOptionDaoEntity buildSimulationOptionDaoEntity(@NonNull String optionCode,
      Integer simulationId, boolean isNew) {
    return SimulationOptionDaoEntity.builder().optionCode(optionCode).simulationId(simulationId)
        .isNew(isNew).build();
  }
  ...
  public void add(@NonNull DocumentRequest documentRequest) {
    ...
    // 資料請求テーブルへ格納
    DocumentRequestDaoEntity documentRequestDaoEntity =
        buildDocumentRequestDaoEntity(documentRequest, customerDaoEntity.getId(), true);
    ...
    for (DocumentRequestProduct product : documentRequest.getDocumentRequestProductList()) {
      SimulationDaoEntity simulationDaoEntity =
          buildSimulationDaoEntity(product, documentRequestDaoEntity.getReceiptNo(), true);
      ...
      if (product.getOptionList().size() != 0) {
        for (String optionCode : product.getOptionList()) {
          SimulationOptionDaoEntity simulationOptionDaoEntity =
              buildSimulationOptionDaoEntity(optionCode, simulationDaoEntity.getId(), true);
