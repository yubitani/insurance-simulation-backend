 ...
 * 資料請求を受け付けるAPIのユースケース
 * ※DB接続周りについて、一旦コメントアウト中
 */
@AllArgsConstructor
public class AddDocumentRequestUsecase {
	...
	public String invoke(@NonNull String inputName, @NonNull String inputNameKana, @NonNull LocalDate inputBirthday,
			@NonNull String inputSex, @NonNull String inputZipCode, @NonNull String inputAddress,
			@NonNull String inputTellNo, @NonNull String inputMailAddress,
			@NonNull List<Simulation> simulationList,
			@NonNull String inputSumInsurancePremium) {
		...
		for (Simulation inputDocumentRequestProductReq : simulationList) {
			documentRequestProductList.add(DocumentRequestProduct.builder()
					...
					.insurancePremium(Integer.parseInt(inputDocumentRequestProductReq.getInsurancePremium())).build());
		}

		// 受け取った情報を元にEntityクラスを生成
		DocumentRequest documentRequest = DocumentRequest.builder().receiptNo(receiptNo).name(inputName)
				.nameKana(inputNameKana).birthday(birthday).sex(sex).zipCode(inputZipCode).address(inputAddress)
				.tellNo(inputTellNo).mailAddress(inputMailAddress)
	...
	private String creatReceiptNo(String telNo) {
		...
		return telNo.substring(telNo.length() - 2) + Double.toString(Math.random() * 10000).substring(0,2);
	}
	
	/**
	 * @author 堀川
	 * TODO 要確認
	 * AddDocumentRequestApiクラスと当クラスのやり取りに使用
	 * AddDocumentRequestReqDtoクラスに使用。
	 */
	@Builder
	@Getter
	@Setter
